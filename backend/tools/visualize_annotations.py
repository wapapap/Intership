...
DEFAULT_DATASET_DIR = os.path.join(PROJECT_ROOT, "datasets/pcb_defect/")
DEFAULT_OUTPUT_DIR = os.path.join(PROJECT_ROOT, "datasets/pcb_defect/vis_output")
...
def load_class_names(dataset_dir: str) -> dict:
    ...
    try:
        ...
        # 列表格式: names: ['name1', 'name2', ...]
        list_match = re.search(r"names:\s*\[([^\]]+)\]", content)
        if list_match:
            names_list = re.findall(r"'([^']*)'|\"([^\"]*)\"", list_match.group(1))
            return {i: (m[0] or m[1]).strip() for i, m in enumerate(names_list)}
        ...
        for line in content.split("\n"):
            ...
            if stripped.startswith("names:"):
                in_names = True
                continue
            if in_names and stripped:
                if stripped[0].isdigit():
                    parts = stripped.split(":", 1)
                    if len(parts) == 2:
                        names[int(parts[0].strip())] = parts[1].strip()
            elif in_names and not stripped:
                break
        return names
    except Exception:
        return {}
